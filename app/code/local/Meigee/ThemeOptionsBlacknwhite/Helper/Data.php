 ...
 public function isNew($product)
{
 ...
 public function prevnext ($product) {
	...
	public function switchNewsletterLocation() {
		...
		if ($newsletterLocation['newsletter']['status'] == 1 and ($newsletterLocation['newsletter']['location'] == 0 or $newsletterLocation['newsletter']['location'] == 2)){
			return 'newsletter/popup_subscribe.phtml';
		}
	}
	
	public function sidebarNewsletter(){
		$newsletter = $this->getThemeOptionsBlacknwhite('meigee_blacknwhite_general');
		if($newsletter['newsletter']['status'] == 1 and ($newsletter['newsletter']['location'] == 1 or $newsletter['newsletter']['location'] == 2)){
	...
	public function setCookie() {
		...
		if ($generalSection['newsletter']['location'] == 0 or $generalSection['newsletter']['location'] == 2){
			return 'js/jquery.cookie.js';
		}
	}
	public function setSingleCookie() {
		$generalSection = $this->getThemeOptionsBlacknwhite('meigee_blacknwhite_general');
		if ($generalSection['newsletter']['location'] == 1){
			$categoryComplete = Mage::getModel('catalog/category')->load(Mage::getModel('catalog/layer')->getCurrentCategory()->getId());
			if($categoryComplete->getMeigeeCatProductsfilter() == 1){
				return 'js/jquery.cookie.js';
			}
		}
	}
